...
export const addFile = action({
  ...
  handler: async (ctx, args) => {
    ...
    const { entryId, created } = await rag.add(ctx, {
      ...
      onComplete: internal.documents.recordUploadMetadata, // Called when the entry is ready (transactionally safe with listing).
    });
    if (!created) {
...
export async function addFileAsync(
  ...
  const { entryId } = await rag.addAsync(ctx, {
    ...
    chunkerAction: internal.documents.chunkerAction,
    onComplete: internal.documents.recordUploadMetadata,
  });
  return {
    url: (await ctx.storage.getUrl(storageId))!,
    entryId,
...
export const listChunks = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.order === "desc") {
      paginatedChunks.page.sort((a, b) => b.order - a.order);
    }
